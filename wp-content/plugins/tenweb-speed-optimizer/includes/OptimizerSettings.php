...
class OptimizerSettings
{
    ...
    private $default_settings = array(
        ...
        'two_disabled_speed_optimizer_pages' => 'robots.txt',
    ...
    public function set_settings($data)
    {
        foreach ($this->settings_names as $key => $val) {
            if (isset($data[$key])) {
              if ( is_array($data[$key]) ) {
                ...
                $this->two_settings[$key] = $this->maybe_change_webp_delivery( $key, $this->two_settings[$key], $checked );
              }
            } else {
        ...
        $this->two_settings["two_connected"] = \Tenweb_Authorization\Login::get_instance()->check_logged_in();
    ...
    public function update_setting($name, $value)
    {
        ...
        if ( is_array( $value ) ) {
          foreach ( $value as $page_id => $critical ) {
            if ( 'front_page' == $page_id ) {
              $page_on_front = array( $page_id => $critical );
              continue;
            }
      ...
      $two_settings["two_connected"] = \Tenweb_Authorization\Login::get_instance()->check_logged_in();
